...
class ElgamalCipher(object):
    ...
    def main(self, mode, key_file, file_to_encrypt_decrypt, out_file):
        filename = out_file  # the file to write to/read from

        if mode == 'encrypt':
            fo = open(file_to_encrypt_decrypt, "r")
            message = fo.read()
            fo.close()
            pubKeyFilename = key_file
            ...
            privKeyFilename = key_file
            FILE_ENCRYPTED = file_to_encrypt_decrypt
            print('Reading from %s and decrypting...' % (FILE_ENCRYPTED))
            decryptedText = self.readFromFileAndDecrypt(FILE_ENCRYPTED, privKeyFilename)
    ...
    def encryptAndWriteToFile(self, messageFilename, keyFilename, message, blockSize=DEFAULT_BLOCK_SIZE):
        ...
        if keySize < blockSize * 8:  # * 8 to convert bytes to bits
            sys.exit(
                'ERROR: Block size is %s bits and key size is %s bits. The Elgamal cipher requires the block size to be equal to or greater than the key size. Either decrease the block size or use different keys.' % (
                blockSize * 8, keySize))
    ...
    def readFromFileAndDecrypt(self, messageFilename, keyFilename):
        ...
        if keySize < blockSize * 8:  # * 8 to convert bytes to bits
            sys.exit(
                'ERROR: Block size is %s bits and key size is %s bits. The Elgamal cipher requires the block size to be equal to or greater than the key size. Did you specify the correct key file and encrypted file?' % (
                blockSize * 8, keySize))
